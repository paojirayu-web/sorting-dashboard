...
const getCachedDefectReasonList = unstable_cache(
    async (startDate: string, endDate: string, category: string, mode: 'scrap' | 'reject') =>
        queryDefectReasonList(startDate, endDate, category, mode),
    ['defect-reason-list'],
    ...
    try {
        ...
        const items =
            searchParams.get('refresh') === '1'
                ? await queryDefectReasonList(startDate, endDate, category, mode as 'scrap' | 'reject')
                : await getCachedDefectReasonList(startDate, endDate, category, mode as 'scrap' | 'reject');
