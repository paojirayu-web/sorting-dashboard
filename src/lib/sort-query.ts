import { getConnection, getSdbConnection, isSdbConfigured, sql } from '@/lib/db';
...
async function getPoolForSource(source: SortSourceId) {
    if (source === 'sdb') return getSdbConnection();
...
export async function querySortSources<T extends Record<string, unknown>>(
    ...
    const settled = await Promise.allSettled(
        sources.map(async (source) => {
            ...
            options?.bind?.(req);
            let query = sqlText.split(SORT_VIEW_TOKEN).join(viewForSource(source));
            if (options?.category) {
            ...
            console.info(`[sort] ${source} ${ms}ms rows=${result.recordset.length}`);
